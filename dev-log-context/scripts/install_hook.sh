#!/usr/bin/env bash
# dev-log-context / install_hook.sh
...
LOADER="$SCRIPT_DIR/load_context.sh"
...
各编辑器安装方式:
  ...
  cursor  bash install_hook.sh --editor cursor  # 写 .cursor/rules/dev-log-context.mdc 规则
卸载:
...
if [ "$GLOBAL" != "1" ]; then
  ...
  REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null)" || {
    echo "[dev-log-context] 不是 git 仓库,无法安装。请 cd 到仓库内,或使用 --global。" >&2
    exit 1
  }
fi
...
remove_hook() {
  ...
  if [ -f "$cfg" ]; then
    local tmp
    tmp="$(mktemp)"
    # 移除所有引用 dev-log-context 的 SessionStart 钩子与该条 hook hook JSON(最简)。
...
hooks["SessionStart"]=[h for h in ss if "dev-log-context" not in json.dumps(h,ensure_ascii=False)]
d["hooks"]=hooks
json.dump(d,open(p,"w"),ensure_ascii=False,indent=2)
PY
      echo "[dev-log-context] 已从 $cfg 移除 SessionStart 钩子"
    else
      echo "[dev-log-context] 需要 python3 才能自动移除,请手动编辑 $cfg 删除引用 dev-log-context 的 SessionStart 钩子"
    fi
  fi
}
...
for h in ss:
    if loader in str(h):
        print(f"[dev-log-context] SessionStart 钩子已存在,跳过"); sys.exit(0)
ss.append({"matcher":"","hooks":[{"type":"command","command":f"bash {loader}"}]})
json.dump(d,open(cfg,"w"),ensure_ascii=False,indent=2)
print("[dev-log-context] 已写入 SessionStart 钩子 → "+cfg)
PY
    else
      echo "[dev-log-context] 需要 python3 合并 JSON,请手动在 $CFG 的 hooks.SessionStart 添加:"
      echo "  { \"matcher\": \"\", \"hooks\": [ {\"type\": \"command\", \"command\": \"bash $LOADER\"} ] }"
    fi
    echo "[dev-log-context] 安装完成:每次会话开始将自动读取近期开发日志。"
    ;;
  ...
  codex)
    ...
    BLOCK="## dev-log-context(自动加载)
每次新会话开始时,先运行: bash $LOADER
以了解项目近期的开发日志,再继续后续任务。"
    if [ -f "$FILE" ] && grep -q "dev-log-context" "$FILE" 2>/dev/null; then
      echo "[dev-log-context] $FILE 已包含 dev-log-context 指令"
    else
      { [ -f "$FILE" ] && cat "$FILE"; printf '\n%s\n' "$BLOCK"; } > "$FILE.tmp" && mv "$FILE.tmp" "$FILE"
      echo "[dev-log-context] 已写入 $FILE"
    fi
    ;;
  ...
  cursor)
    ...
    mkdir -p "$RULEDIR"
    FILE="$RULEDIR/dev-log-context.mdc"
    cat > "$FILE" <<EOF
---
description: 会话开始时自动加载近期开发日志
globs:
alwaysApply: true
---
每次新会话开始 / 打开新对话时,先运行: bash $LOADER
读取项目近 7 天(或最近 10 条)的开发日志,用于了解项目上下文与代码风格,再继续任务。
EOF
    echo "[dev-log-context] 已写入 Cursor 规则 $FILE"
    ;;

  *)
    echo "[dev-log-context] 未知编辑器:$EDITOR(支持 claude / codex / cursor)" >&2
    exit 1
    ;;
esac
